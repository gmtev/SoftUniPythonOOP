...
class Hotel:
    ...
    def take_room(self, room_number: int, people: int) -> None:
        try:
            ...
            return  # since no output is wanted

        result = room.take_room(people)

        if not result:  # no output due to successfully booking the room
            self.guests += people
    ...
    def free_room(self, room_number) -> None:
        ...
        if not result:  # no output due to successfully freeing the room
            self.guests -= people
